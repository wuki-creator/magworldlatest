"""Train and validate reciprocal MagWorld on true zero-shot H1 targets."""
...
def metric_objective(metrics: dict[str, float]) -> float:
    return (
        metrics["cosine"]
        + 0.25 * metrics["direction_fidelity"]
...
def main() -> None:
    ...
    WorldModelH1V3 = importlib.import_module("model_world_h1_v3").WorldModelH1V3
    model_config = {
        "n_genes": len(genes),
    ...
    model = WorldModelH1V3(**model_config).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        totals = {"loss": 0.0, "global": 0.0, "de": 0.0, "cosine": 0.0,
                  "direction": 0.0, "rank": 0.0, "pds": 0.0}
        batches = 0
